defmodule Marvin.Puppy.Perception do
	...
	def perceptor_configs() do
		[
			...
			PerceptorConfig.new(
				name: :hungry,
				generates: [:hungry],
				focus: %{senses: [:time_elapsed], motives: [], intents: [:eat, :go_forward, :go_backward]},
				span: {1, :mins},
				ttl: {30, :secs},
				logic: hungry()),
			# A food perceptor
			...
			PerceptorConfig.new(
				name: :scent,
				generates: [:scent_strength, :scent_direction, :time_elapsed],
  ...
  def used_senses() do
	...
	def food() do
		fn						 
			(_percept, %{percepts: []}) -> nil
			(%Percept{about: :color, value: @food_color}, %{percepts: percepts}) ->
				if latest_memory?(
							percepts,
							:ambient,
	...
	def collision() do
		fn
			...
			(%Percept{about: :touch, value: :pressed}, _memories) ->
				...
				(%Percept{about: :distance, value: n}, %{percepts: percepts}) when n < 20 ->
				if not any_memory?(
							percepts,
							:distance,
							1000,
	...
	def danger() do
		fn
			...
			(%Percept{about: :time_elapsed, value: %{count: count}}, %{percepts: percepts}) ->
			if rem(count, 3) == 0 do # check every three seconds if danger is gone
				if not any_memory?(
							percepts,
							:danger,
							3000,
	...
	def hungry() do
		fn # Hunger based on time since last :eat intend
			(%Percept{about: :time_elapsed, value: %{count: count}}, %{intents: intents}) ->
			if rem(count, 3) == 0 do # Check if hungry every 3 ticks
				# How much did I eat in the last 30 secs?
				how_full = summation(intents,
														 :eat,
														 30_000,
					fn(value) ->
						case value do
				...
				forward_count = summation(intents, :go_forward, 30_000, fn(_value) -> 1 end, 0)
				backward_count = summation(intents, :go_backward, 30_000, fn(_value) -> 1 end, 0)
				exercized? = (forward_count + backward_count) > 0
				if not exercized?, do: Logger.info("NO EXERCIZE!")
				#How much did I exercize
				cond do
					how_full < 5 and exercized? ->
						Percept.new(about: :hungry, value: :very)
					how_full < 10 and exercized? ->
						Percept.new(about: :hungry, value: :a_little)
					true -> 
						Percept.new(about: :hungry, value: :not)
				end
			else
				nil
			end
	...
	def scent() do
        ...
        latest_scent_strength = last_memory(
				...
				:scent_strength,
				fn({_, c}) -> c == n end)
			latest_strength_value = case latest_scent_strength do
																nil ->
																	nil
																%Percept{about: :scent_strength, value: {value, _}} ->
																	value
															end
			beacon_on? = heading != 0 or any_memory?(percepts,
																							 {:beacon_distance, n},
																							 3000,
				fn(value) -> value > 0 end)
			if beacon_on? do		# beacon channel was detected so heading is valid				 
				cond do
					heading < -10 ->
						Percept.new(about: :scent_direction, value: {:left, abs(heading), latest_strength_value, n})
					heading > 10 ->
						Percept.new(about: :scent_direction, value: {:right, abs(heading), latest_strength_value, n})
					true ->
						Percept.new(about: :scent_direction, value: {:ahead, abs(heading), latest_strength_value, n})
				end
			else
				nil
			end			
			(%Percept{about: :time_elapsed, value: %{count: count}}, %{percepts: percepts}) ->
				if rem(count, 3) == 0 do # every 3 ticks
					new_percepts = Enum.reduce(1 .. max_beacon_channels(),
																		 [],
						fn(channel, acc) ->
							no_recent_smell? = not any_memory?(percepts,
																								 :scent_strength,
																								 5000,
								fn({_strength, c}) -> c == channel end)
						if no_recent_smell? do
							Logger.info("NO OLD SCENT for #{channel}")
							[Percept.new(about: :scent_strength, value: {:unknown, channel}) | acc]
						else
							acc
						end
						end)
					case new_percepts do
						[] -> nil
						_ -> new_percepts
					end
				else
	...
	def food_nearby() do
		fn
			(%Percept{about: :scent_strength, value: {strength, channel}}, %{percepts: percepts}) ->
				other_eating? = any_memory?(percepts,
																		:other_eating,
																		5_000,
			...
			cond do
				strength in [:strong, :very_strong] and (channel == 1 or other_eating?) ->
    ...
    fn
      (%Percept{about: :heard, value: %{info: %{doing: :eating}, id_channel: id_channel}}, %{percepts: percepts}) ->
        ...
        Percept.new(about: :other_eating,
			...
			(%Percept{about: :time_elapsed, value: %{count: count}}, %{percepts: percepts}) ->
			if rem(count, 3) == 0 do # check every 3 seconds
